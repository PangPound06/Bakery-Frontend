...
export default function CakePage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch("https://bakery-backend-production-6fc9.up.railway.app/api/products/category/cake");
        if (response.ok) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50" style={{ fontFamily: "'Prompt', sans-serif" }}>
      <section className="relative h-[45vh] sm:h-[50vh] overflow-hidden">
        <img src="https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=1600&q=80" alt="Cake" className="w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 left-0 right-0 px-5 sm:px-8 lg:px-16 pb-10 sm:pb-14">
          <div className="max-w-7xl mx-auto">
            ...
            <p className="text-black text-sm mt-2">เค้กสุดพิเศษ สำหรับทุกโอกาส</p>
          </div>
        </div>
      </section>
